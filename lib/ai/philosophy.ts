...
export const HYBRID_GUIDANCE = `Hybrid sessions simulate the HYROX format: threshold-pace runs interleaved with non-running cardio/strength stations ("events"). These sessions ARE the program's compromised-running work: running immediately off a weighted/erg station trains the athlete to run well on pre-fatigued legs - the race-specific durability HYROX demands. (The weekly long run is a separate, straightforward aerobic long run and must NOT have stations threaded into it.)
