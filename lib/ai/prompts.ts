...
- Lift:   { "kind": "lift", "liftType": "upper|lower|full|power", "movements": [ { "pattern": "squat|hip_hinge|lunge|horizontal_press|vertical_press|horizontal_pull|vertical_pull", "sets": <int>, "repRange": "string" } ] }  (the engine sets sets/reps/intensity/RIR and any plyometric element deterministically — just choose which patterns go in each session)
...
- Match each run's runType, each lift's liftType and each session's goalZone to what the skeleton specifies for that slot. A "power" lift slot is an explosive day — return liftType "power" for it, not "full".
- Hybrid content is built by the engine (every race station at a trainable dose, each preceded by a full race-distance run). Return the hybrid slot with whatever elements you like; it will be replaced.
